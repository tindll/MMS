...
with open('ALLtrades.json','r') as tradesJSON1:
...
def create_plot(df):
    ...
    mpf.plot(df, type='line', axtitle = symbol+" "+timeframe +" ("+amountDays+"D)", xrotation=20, datetime_format=' %A, %d-%m-%Y', savefig='chart'+tradeID+'.png', volume = True, volume_panel=2, style = s,addplot=ap0, fill_between=dict(y1=df['BB_LOWER'].values, y2=df['BB_UPPER'].values, alpha=0.15))
...
def updateJSON_newtrade(symbol,positionType,openPrice,close,leverage,Date,tradeReason,trade_ID):
    with open('ALLtrades.json','r+') as tradesJSON:
